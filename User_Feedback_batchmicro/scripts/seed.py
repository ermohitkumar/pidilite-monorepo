...
Run once after `alembic upgrade head` to bootstrap:
  ...
  - Seed Dummy Products for LLM catalog context
...
from db.models import User, AppConfig, KeywordDictionary, Product
db = SessionLocal()
...
if not db.query(User).filter(User.email == "admin@pidilite.com").first():
    admin = User(
        ...
        password_hash="fake_hash_password123",        # Swapped to the password we tested
        full_name="Pipeline Admin",
        role="super_admin",                           # <--- Set to super_admin to test your RBAC!
        is_active=True,
    )
    db.add(admin)
...
# ── App config defaults ───────────────────────────────────────────────────────
defaults = [
    ("batch_size",              {"value": 10},    "Number of jobs per STT batch"),
    ...
    ("translation_target_language", {"value": "en"}, "Target language for STT transcript translation"),
    ("stt_language_code",       {"value": "en-IN"}, "Default STT language"),
    ("max_retry_count",         {"value": 2},       "Max retries before FAILED"),
]
...
# ── Dummy Products Seed Removed ───────────────────────────────────────────────
# The products are now seeded via scripts/scrape_and_seed.py using the real catalog
