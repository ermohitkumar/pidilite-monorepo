    ...
    for file in files:
        if file.endswith(".py"):
            ...
            for test_name, expected_status in fixes.items():
                ...
                if match:
                    block = match.group(1)
                    # Replace both res.status_code and resp.status_code and response.status_code
                    new_block = re.sub(r"assert (res|resp|response)\.status_code == 200",
                                       rf"assert \1.status_code == {expected_status}", block)
                    content = content.replace(block, new_block)

            # Fix "detail" -> "message"
            content = content.replace('["detail"]', '["message"]')
